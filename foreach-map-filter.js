...
function doubleValues(arr){
    //let returnArr = [];
    return arr.map(function(val, ind, passedArr){
        return val*2;});
       // return returnArr;
}
...
function onlyEvenValues(arr){
    return arr.filter(function(value, index, passedArr){
        if(!(value %2)){
            return value;
        }
    });
...
function addKeyAndValue(arr,key,value){
     ...
     arr.forEach(function(val, index, pushedArr){
...
function vowelCount(str){
    let tempStr = '';
    tempStr+=str;
    let strArr = Array.from(tempStr.toLowerCase());
    
    let returnObj = {};

    let vowelsOnly = strArr.filter(function(val, index, pushedArr){
    ...
    for(char of vowelsOnly){
        if(!(returnObj[char])){
            let charCounter =0;
            for(charTwo of vowelsOnly){
                if (charTwo === char){
                    charCounter++;
                }
...
function doubleValuesWithMap(arr) {
    return arr.map(function(val, index, pushedArr){
...
function filterByValue(arr, key) {
     ...
     return arr.filter (function(val, index, pushedArr){
...
function find(arr, searchValue) {
    ...
    let finalVal =parseInt(arr.filter (function(val, index, pushedArr){
        

         if(arr.indexOf(searchValue)!=-1 && searchValue ==val){
            return searchValue;
...
function findInObj(arr, key, searchValue) {
    ...
    let tempArr = [];
    tempArr=arr.filter (function(val, index, pushedArr){
      
        

        if(val[key] && val[key]===searchValue){
...
function removeVowels(str) {
   ...
   let filteredArr =[];
   filteredArr= tempArr.filter(function(val, index, pushedArr){
...
function doubleOddNumbers(arr) {
   ...
   let tempArr = arr.filter(function(val, index, pushedArr){
        if(val%2){
            return true;
        }
        else return false;
    ...
    return(tempArr.map(function(val, index, pushedArr){
        return val*2;
